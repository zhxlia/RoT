...
def list_to_aligned_md(table: List[List[str]]) -> str:
    if isinstance(table[0], dict) and "caption" in table[0].keys():
        result += "## " + ", ".join(table[0]["caption"].split(" | ")) + "\n"
    if isinstance(table[0], dict):
        table = table[0]["table"]
...
def trans_table(org_table: List[Any], typee: str, truncated: bool = False):
    ...
    if "aligned_md" in typee:
        tr_table = list_to_aligned_md(org_table)   
    elif "md" in typee:
...
if __name__ == '__main__':
    with open("./inference/results/Analysis/multi_traversal_summary.json", "r", encoding="utf8") as f:
        data: List[Dict[str, Any]] = json.load(f)

    for d in data:
        # WikiTQ-train-21
        # d["table"] = str(Table(d["table"][0]["table"], 'information', "sql"))
        if d["id"] == "WikiTQ-test-217":
            d["table"] = list_to_md(d["table"])
            # d["table"] = str(Table(d["table"][0]["table"], 'information', "sql"))
            # print(d["table"])
            print("\n".join(d["output"]))
            # print("\n".join(d["output_long"]))
