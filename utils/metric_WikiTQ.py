...
def evaluate_one_instance(pred: List[str], gold: List[str]) -> float:
    pred_answer_val = to_value_list(deepcopy(pred))
    gold_answer_val = to_value_list(deepcopy(gold))
    return 1.0 if check_denotation(pred_answer_val, gold_answer_val) else 0.0
